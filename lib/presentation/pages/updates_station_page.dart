    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        return Scaffold(
          appBar: AppBar(
            title: Text(
              "${context.tr('post')} ${snapshot.requireData.updatesStationPageEntity.stationId}",
            ),
            actions: [
              IconButton(
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        final availableVersions =
            snapshot.requireData.updatesStationPageEntity.availableVersions;
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        final availableStations =
            snapshot.requireData.updatesStationPageEntity.availableStations;
        final currentVersionOnServer =
            snapshot
                .requireData
                    ...
                    child: ElevatedButton(
                      onPressed:
                          availableStations.length > 0
                                ...
                                try {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBars.getSuccessSnackBar(
                                      message: context.tr(
                                        'copying_from_buffer_in_progress',
                                      ),
                                    ),
                                  );
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBars.getErrorSnackBar(
                                      message:
                                          "${context.tr('error_has_occurred')} $e",
                                    ),
                                  );
                                } catch (e) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBars.getErrorSnackBar(
                                      message:
                                          "${context.tr('an_unknown_error_has_occurred')} $e",
                                    ),
                                  );
                                }
                              }
                              : null,
                      child: Text(
                        context.tr('set_version_from_server_and_reboot'),
                      ),
                    ),
                  ),
                ],
                    ...
                    onPressed: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
                          SnackBars.getSuccessSnackBar(
                            message: context.tr(
                              'the_reboot_task_has_been_created',
                            ),
                          ),
                        );
                      } on FormatException catch (e) {
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
                          SnackBars.getErrorSnackBar(
                            message: "${context.tr('error_has_occurred')} $e",
                          ),
                        );
                      } catch (e) {
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
                          SnackBars.getSuccessSnackBar(
                            message: context.tr(
                              'the_task_to_hash_the_versions_from_the_post_has_been_created',
                            ),
                          ),
                        );
                      } on FormatException catch (e) {
                    ...
                    label: Text(
                      "${context.tr('to_get')} ${context.tr('information_about_all_post_versions_and_display')}",
                    ),
                  ),
                ),
              ],
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
                          SnackBars.getSuccessSnackBar(
                            message: context.tr(
                              'the_task_to_download_the_latest_version_has_been_created',
                            ),
                          ),
                        );
                      } on FormatException catch (e) {
                    ...
                    label: Text(
                      context.tr(
                        'download_version_from_github_build_local_and_reboot',
                      ),
                    ),
                  ),
                ),
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      try {
                        ...
                        ScaffoldMessenger.of(widgetContext).showSnackBar(
                          SnackBars.getSuccessSnackBar(
                            message: context.tr(
                              'the_task_to_create_a_new_version_has_been_created',
                            ),
                          ),
                        );
                      } on FormatException catch (e) {
                    ...
                    label: Text(
                      context.tr('copy_to_post_current_version_and_reboot'),
                    ),
                  ),
                ),
              ],
