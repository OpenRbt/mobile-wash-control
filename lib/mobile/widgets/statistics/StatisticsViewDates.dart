...
class StatisticsViewDates extends StatelessWidget {
  final Repository repository;
  final DateTimeRange dateTimeRange;

  Map<int, StationMoneyReport?> _moneyReports = Map();

  StatisticsViewDates({super.key, required this.dateTimeRange, required this.repository});
  ...
  Future<void> _loadMoneyReports(BuildContext context) async {
    ...
    List<Future> futures = List.generate(12, (index) async {
      final res = await repository.getStationMoneyReportsByDates(index + 1, dateTimeRange.start, dateTimeRange.end, context: context);
    ...
    // for (int i = 1; i <= 12; i++) {
    //   final res = await repository.getStationMoneyReportsByDates(i, dateTimeRange.start, dateTimeRange.end, context: context);
    //   _moneyReports[i] = res;
    //
    //   if (res != null) {
    //     total.banknotes = (total.banknotes ?? 0) + (res.banknotes ?? 0);
